...
bool mm_checkheap(int lineno)
{
    void *bp = header_to_payload(heap_listp);
    ...
    if ((*(unsigned int *)(bp-wsize)) != (*(unsigned int *)(bp+get_size(heap_listp))-dsize))
    {
        printf("Error: header does not match footer\n");
        return false;
    }

    for (bp = heap_listp; get_size(bp-wsize) > 0; bp = bp+get_size(bp-wsize)) {
       if ((size_t)bp % 8)
            printf("Error: %p is not doubleword aligned\n", bp);
       if (*(unsigned int *)(bp-wsize) != *(unsigned int *)((bp+get_size(heap_listp))-dsize))
            printf("Error: header does not match footer\n");
    ...
    if ((get_size(bp-wsize) != 0) || !(*(unsigned int *)(bp-wsize)))
