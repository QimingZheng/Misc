...
int main(int argc, char *argv[]) {
  ...
  float *d_a, *d_b;
  checkCudaErrors(cudaMalloc((void **)&d_a, sizeof(float) * mat_size));
  checkCudaErrors(cudaMalloc((void **)&d_b, sizeof(float) * mat_size));

  // copy matrix A from host to device memory
  checkCudaErrors(cudaMemcpy(d_a, h_a, sizeof(float) * mat_size, cudaMemcpyHostToDevice));
